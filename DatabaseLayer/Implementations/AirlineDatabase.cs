...
namespace DatabaseLayer.Implementations
{
    public class AirlineDatabase : IAirlineDatabase
    {
        ...
        public Flight GetFlight(int id)
        {
            ...
            using (var context = new DataContext(DataContext.ops.dbOptions))
            {
                flight = context.Flight.Where(theFlight => theFlight.FlightId == id).SingleOrDefault();
            }
        ...
        public bool DeleteFlight(Flight flight)
        {
            ...
            using (var context = new DataContext(DataContext.ops.dbOptions))
            {
                Flight flightFromDb = context.Flight.SingleOrDefault(theFlight => theFlight.FlightId == flight.FlightId);
                context.Flight.Remove(flightFromDb);

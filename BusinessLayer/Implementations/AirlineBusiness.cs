...
namespace BusinessLayer.Implementations
{
    public class AirlineBusiness : IAirlineBusiness
    {
        ...
        public Holder<Flight> DeleteFlight(Flight flight)
        {
            Flight flightFromDb = _airlineDatabase.GetFlight(flight.FlightId);

            if (flightFromDb.FlightId <= 0)
                return CheckFlight(flight, 404, "Flight you're trying to delete isn't found");

            if (_airlineDatabase.DeleteFlight(flight))
                return CheckFlight(flight, 200, "");

            return CheckFlight(flight, 400, "Unable to remove flight");
        }
